...
// v0.4.x P4:attach 进行中集合——attachToReader 是 async（先 await
// _initPromise + waitFor iframe 最长 20s），attached.set 在函数末尾。若无此
// 集合，2s 轮询会在 await 期间重入，同一 reader 并发跑两个 attach → 同一
// 窗口装 2 份 keydown 监听（发音快捷键双播根因之一）。入口占位、完成清除。
const attaching = new Set<object>();

// v0.4.x P4:插件已禁用标志——进行中的 attach 完成后不再记录 attached
// （防 cleanupAll 之后残留监听引用）。
let disposed = false;
...
export function initHoverTranslate() {
  ...
  disposed = false; // v0.4.x P4:重新启用时重置禁用标志
  initThemeWatcher();
...
function attachToAllReaders() {
  ...
  for (const reader of readers) {
    // v0.4.x P4:已附加或 attach 进行中 → 跳过。attaching 防止 async
    // attachToReader 在 await 期间被 2s 轮询重入（根除双份 keydown 监听）。
    if (attached.has(reader) || attaching.has(reader)) continue;
    attaching.add(reader);
    attachToReader(reader)
      .catch((e) => dbg(`attach failed: ${e}`))
      .finally(() => attaching.delete(reader));
  }
}
...
export function cleanupAll() {
  disposed = true; // v0.4.x P4:禁用期间完成 attach 的任务不再记录
  if (pollTimer != null) {
    try {
...
async function attachToReader(reader: _ZoteroTypes.ReaderInstance) {
  ...
  const onMouseUp = (ev: MouseEvent) => {
    try {
      ...
      // Bug B: 翻译统一用连字展开后的词(ﬁ→fi),与 hover 路径一致。
      const clickWord = expandLigatures(hit.word);
      lastWord = clickWord;
      ...
      void doTranslate(win, reader, clickWord, lastWordRef, contextLineRef, hit.range);
    } catch {
      /* suppress */
    }
  };
  ...
  const onKeyDown = (ev: KeyboardEvent) => {
    try {
      ...
      // Bug B: 翻译统一用连字展开后的词(ﬁ→fi),与 hover 路径一致。
      const modWord = expandLigatures(hit.word);
      lastWordRef.set(modWord);
      if (getPref("enableHighlight")) {
        void highlightHit(activeWinRef.win, reader, hit);
      }
      void doTranslate(activeWinRef.win, reader, modWord, lastWordRef, contextLineRef, hit.range);
    } catch {
      /* suppress */
    }
  };
  ...
  if (disposed) {
    // 插件已禁用（cleanupAll 已执行）→ 立即清理本次附加的监听并放弃记录
    try {
      cleanup();
    } catch { /* ignore */ }
...
function onReaderMouseMove(
  ...
  try {
    ...
    // Bug B fix: expand ligatures (ﬁ→fi, ﬂ→fl, …) for translation / dedup /
    // popup purposes. The raw hit.word (with ligatures) is retained for
    // highlight + range operations — C-channel normalizeWord() expands
    // ligatures too, so matching still works; range.toString() must match
    // the raw word for the consistency check in getWordAtPoint.
    const expandedWord = expandLigatures(hit.word);
    ...
    if (highlightEnabled) {
      ...
      // Bug B 修复点2(C 通道校正):DOM 取到半截词(textLayer 连字/span 拆分)
      // 但 C 通道定位到完整词时,用 located.word 校正翻译词——翻译不再只取
      // 半个词。仅当用户仍停留在该词上(lastWordRef 未变)才校正,避免快速
      // 滑过多个词时错误触发。schedule 内部 debounce 会取消旧 timer,
      // 校正词重新计时,用户无感。
      void highlightHit(innerWin, reader, hit, ev.clientX, ev.clientY).then(
        (located) => {
          if (located?.word && lastWordRef.get() === expandedWord) {
            const corrected = expandLigatures(located.word);
            if (corrected && corrected !== expandedWord) {
              lastWordRef.set(corrected);
              schedule(corrected);
            }
          }
        },
    ...
    if (expandedWord === lastWordRef.get() && popupShown(innerWin, expandedWord)) {
      return;
    }
    lastWordRef.set(expandedWord);

    schedule(expandedWord);
  } catch (e) {
...
function getWordAtPoint(
  ...
  if (!text) return null;
  // 借鉴 Zotero reader.js _getTouchAnnotationStartPosition：用 Intl.Segmenter
  // （UAX#29）精确判断 caret offset 所在 segment 是否为词。词边缘/行间距
  // 场景 caret 落在空格/标点 segment → snapWordAtOffset 返回 null（不误取
  // 相邻词），从源头解决问题A的词边缘误取。Intl.Segmenter 不可用时降级
  // 到正则 wordRangeAtOffset。
  // v0.4.1: caret 落在行尾连字符 '-' 上时（如 "ob-" 的 '-'），正则
  // 匹配不到字母段 → 向前取前一段字母（"ob"），后续跨行重组补全为完整词。
  const segLang = node.parentElement?.lang || undefined;
  let wr = snapWordAtOffset(text, cp.offset, segLang);
  ...
  try {
    ...
    if (wr.start === 0) {
      ...
      if (span) {
        ...
        if (prevSpan) {
          ...
          if (prevText && isWordChar(prevText[prevText.length - 1])) {
    ...
    if (wr.end === text.length) {
      ...
      if (span) {
        ...
        if (nextSpan) {
          ...
          if (trimmed && isWordChar(trimmed[0])) {
    ...
    // 问题A修复（v0.4.x P3）：checkXYInElement 二次校验——扩展后的
    // wordRange 的 bounding rect 必须真正包含鼠标点，否则丢弃。
    // 参考 MouseTooltipTranslator mouseover.js:373-386 的 checkXYInElement：
    // 词边缘/行间距处 caretPositionFromPoint 可能命中相邻词（WebKit bug
    // #30034/#30689 在 zoom/scroll 时 caretRangeFromPoint 返回错误结果），
    // 此校验从源头丢弃"鼠标不在该词 bounding box 内"的误取（报告 §3.5、
    // §6.2 修复点3）。跨行重组词（两段 rect）取任一 rect 包含鼠标即通过。
    try {
      const rects = range.getClientRects();
      if (rects && rects.length > 0) {
        const hitOk = Array.from(rects).some(
          (r) => x >= r.left && x <= r.right && y >= r.top && y <= r.bottom,
        );
        if (!hitOk) return null;
      }
    } catch {
      /* 浏览器不支持 getClientRects 时跳过（罕见） */
    }
...
async function highlightHit(
  ...
  try {
    ...
    // 阶段一(B 修复):同时传入 A 通道 range 的视口 rects(range.getClientRects)
    // ——word-locator 用它们做行归属判定的浏览器权威几何(aRowAnchor),
    // 消除 viewBox/convertToPdfPoint 换算误差对选行的影响。
    const result = await locateWord({
      reader,
      innerWin,
      word: hit.word,
      range: hit.range,
      rangeRects: rangeViewportRects(hit.range),
      mouseX,
      mouseY,
    });
    if ((innerWin as any).__hteHighlightSeq !== seq) return null; // 过期,丢弃
...
function shouldKeepHighlightInGap(
  ...
  try {
    ...
    // 问题A修复（v0.4.x P3）：防跨行粘附——若鼠标已移到词外包矩形的
    // 垂直范围之外（且距离超过一行），不再保持。旧实现只做矩形外扩
    // 判定，行间距处的鼠标（上一行词下方 / 下一行词上方）会错误保持
    // 上一行高亮（报告 §3.3 根因）。
    const boxH = Math.max(1, bottom - top);
    const midY = (top + bottom) / 2;
    const dy = Math.abs(clientY - midY);
    if (dy > boxH * 1.5) return false; // 垂直超出词盒 1.5 倍行高 → 视为已移行
    return (
      clientX >= left - PAD &&
      clientX <= right + PAD &&
      clientY >= top - PAD &&
