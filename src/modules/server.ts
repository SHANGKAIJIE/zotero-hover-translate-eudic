/**
 * HTTP endpoint registration for Shanbay integration.
 *
 ...
 *   POST /shanbay-token/token — receive auth_token from HTE Bridge
 */
...
class TokenEndpoint {
  ...
  init(data: any, sendResponse: Function) {
    // Zotero's server may pass data as a pre-parsed object (when
    // supportedDataTypes includes "application/json") or as a string.
    let body: any;
...
export function registerServer(): void {
  Zotero.Server.Endpoints["/shanbay-token/ping"] = PingEndpoint;
  Zotero.Server.Endpoints["/shanbay-token/token"] = TokenEndpoint;
  try {
    Zotero.debug("[hovertranslateeudic/server] /shanbay-token/* registered");
  } catch { /* ignore */ }
}
...
export function unregisterServer(): void {
  ...
  try {
    Zotero.debug("[hovertranslateeudic/server] /shanbay-token/* unregistered");
  } catch { /* ignore */ }
}
