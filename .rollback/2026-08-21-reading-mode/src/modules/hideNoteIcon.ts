 ...
 *
 * 渲染机制（双版本兼容，实际运行源码 resource/reader/reader.js）：
 *  - Zotero 9 / Firefox ESR 140：
 ...
 *  - Zotero 10（reader.js 大重构）：
 *      canvas 路径：Page._buildDisplayList()（原 _renderCommon）中
 *        _pushNote(items, annotation)          —— 独立 Note 的便签图标
 *        _pushCommentIcons(items, annotations) —— 高亮/下划线/图片中带 comment 的便签图标
 *        （方法挂在 Page 类上；页面 _pages[] 中即 Page 对象，page._layer = PDFView，
 *         已集成原 AnnotationLayer 职责）
 *      DOM React 路径：DOMView.setAnnotations()（EPUB/snapshot，PDF 不经此渲染便签）
 *
 * 隐藏策略（双路径同时 patch，按原型方法存在性自动探测版本）：
 *  1. DOM React 路径：包装 setAnnotations（Zotero 9 AnnotationLayer / Zotero 10 DOMView），
 *     对需要隐藏的 annotation 以副本将 comment 置空（不影响弹窗数据），使组件不渲染
 *     CommentIcon；恢复时用原始数据重新 setAnnotations。
 *     （守卫：Zotero 10 的 PDFView 虽有 setAnnotations 但无 _renderAnnotations，
 *       不属 DOM React 渲染层，跳过以免污染 PDFView._annotations）
 *  2. canvas 路径：包装 Zotero 9 的 _drawNote/_drawCommentIcons 与 Zotero 10 的
 *     _pushNote/_pushCommentIcons，直接过滤需要隐藏的 annotation；恢复原型后重绘。
 *  3. 重绘：Zotero 9 用 _invalidateSignature() + render()；Zotero 10 的 Page.render()
 *     自带 display-list signature 比较，直接调用即可，另触发 page._layer._render()。
 *
