...
[![Version](https://img.shields.io/badge/version-0.3.1-orange)](https://github.com/SHANGKAIJIE/zotero-hover-translate-eudic/releases)
...
- **加词同步创建 PDF 注释**：将单词加入生词本时，可自动在 PDF 上创建高亮/下划线注释，支持翻译写入评论或正文、自动添加标签（v0.2.8）。
- **隐藏便签图标**：按需隐藏 PDF 中高亮/下划线/图片注释的便签图标（支持仅本插件注释 / 全部文本注释两种范围），独立便签图标可单独控制，注释 ID 自动跟踪与清理（v0.2.8）。
- **弹窗自动关闭**：可设置弹窗自动关闭延时（0 = 不自动关闭）。
- **PDF 右侧生词本面板**：开启后点击 PDF 右侧工具栏的本插件图标即可显示生词本面板（仅本地生词表 / Zotero 笔记平台可用）。面板顶部支持隐藏释义、隐藏音标、放大/缩小字体、清空；单词卡片支持点击跳转原文、编辑与删除，且与本地 CSV / Zotero 笔记**双向同步**（v0.3.1）。
...
**开启右侧信息栏的生词本面板**：勾选后，在主窗口条目面板与 PDF 阅读器的**右侧信息栏**出现本插件面板（含信息栏图标按钮），点击图标即可显示 / 隐藏生词本面板。

> 提示文字「仅当生词本平台为本地生词表、Zotero 笔记时可使用」始终显示；但信息栏面板与开启按钮**仅在生词本平台为本地生词表 / Zotero 笔记 且勾选本开关时启用**（平台为欧路 / 扇贝 / 墨墨时信息栏面板与开启按钮不启用）。切换生词本平台时启用状态**实时更新**，无需取消勾选或重启 Zotero。

面板功能：
- **顶部工具栏**：`当前条目/所有条目` 视图切换、`译` 隐藏释义、音标图标隐藏音标、播放图标隐藏发音按钮、`序` 排序、`A+` 放大字体 / `A-` 缩小字体（9–24px）、`清空`（需二次确认）。
- **视图切换**：`当前条目`（默认）仅显示所选条目的生词；`所有条目` 显示全部生词（该模式下按钮高亮）；切换状态跨重启记忆。
...
**加入生词本时同步添加到注释**：勾选后，将单词加入生词本时同步在 PDF 上创建 Zotero 注释（该板块的总开关，不勾选时下方所有设置均为灰色不可用）。支持悬停翻译（hover）和选区弹窗（selection）两种场景：
...
- **隐藏范围**：
  - `仅隐藏单词文本的便签`：仅隐藏**本插件创建**的注释便签（按注释 ID 精确识别，自动记录于 `annotationTrackedIDs`；不受标注颜色、文本、标签影响，无启发式误判）
