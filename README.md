...
**启用悬停翻译**：控制插件的取词翻译功能是否启用。取消勾选后，取词触发方式 / 悬停延迟 / 弹窗关闭 / 显示模式均变为灰色不可操作；其中「修饰键组合」与「悬停触发延迟」两行在对应触发方式下会**直接隐藏**（而非置灰）。
...
**划词时禁用悬停弹窗（避免与划词翻译弹窗冲突）**：该行为为**固定默认功能**（默认启用，不在设置面板中展示、不可关闭）——在阅读器中划词选择文本时，抑制悬停翻译弹窗，避免与 Translate for Zotero 的划词翻译弹窗重叠冲突。
...
**加入生词本时同步添加到注释**（生词注释卡片总开关）：勾选后，将单词加入生词本时同步在 PDF 上创建 Zotero 注释。支持悬停翻译（hover）和选区弹窗（selection）两种场景。**取消勾选时，卡片内其余设置置灰不可操作**（参考「启用悬停翻译」总开关模式）。

**加入术语库时同步添加到注释**（术语注释卡片总开关）：勾选后，将术语加入术语库时同步在 PDF 上创建 Zotero 注释；取消勾选时同样将卡片内其余设置置灰。

生词注释与术语注释各自拥有**完全独立的一套设置**（互不干扰）：自动翻译 / 翻译保存位置 / 原文保存位置 / 分隔方式 / 分隔符 / 标注方式 / 标注颜色 / 自动添加标签 / 标签名称。以下以生词注释为例说明（术语注释对应项为「术语标注方式 / 术语标注颜色 / 术语标签名称」等）：
...
**翻译后自动为注释添加标签**：勾选后，为注释自动添加标签，标签名称可在后方输入框设置（生词默认 `单词`，术语默认 `术语`）。标签以**覆盖式**方式设置——插件创建的注释标签始终仅为该标签名，不受其他插件（如 Actions and Tags for Zotero、zotero-style 的颜色标签联动）自动附加标签的影响。
