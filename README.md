...
- **弹窗位置可调**：翻译弹窗可选择显示在单词上方或下方（默认上方），紧贴文本 8px 间距，空间不足自动翻转；译文/释义填充后自动重新定位保持间距恒定，完整模式下字典释义区始终位于远离单词的一侧、核心区（单词/译文/+按钮）永远贴近单词（v0.2.9）。
...
**弹窗位置**：选择翻译弹窗相对单词原文的位置——`单词上方`（默认）或 `单词下方`，弹窗紧贴文本（8px 间距），左边缘与单词左边缘对齐；空间不足时自动翻转到另一侧，越界时收进视口。**间距恒定**：译文 / 字典释义填充后弹窗自动重新定位，与单词始终保持 8px 间距（多行译文不会向下延伸盖住单词）。**完整模式布局跟随弹窗方向**：字典释义区始终位于远离单词的一侧（弹窗在上方时释义在弹窗上部、在下方时释义在弹窗下部），单词/译文/+按钮 核心区永远贴近单词，分割线始终位于释义与核心区之间。
...
> 字典引擎的「首条释义」提取已适配 Translate 插件全部词典源（必应 / 有道 / 海词 / 剑桥 / 科林斯 / FreeDictionary / Weblio / Gramota），自动跳过音标行、词性行（含 `noun[C]` 变体）、英文定义行与区块标题（如 Weblio 的「意味・対訳」），剥离 `网络释义` 前缀后取第一条中文释义。字典引擎**直接查询鼠标悬停的原文词，不做词形还原**——词形还原仅作用于「加生词本」流程（由下方「词形选择」控制），确保悬停翻译始终查当前单词（如 imager 不会被还原为 imag）。
...
- **划词过程中**（按住鼠标拖选文本）：悬停翻译弹窗与取词高亮均不显示，避免干扰划词手势。
- **划词完成后**（选区工具栏 / Translate 划词翻译弹窗显示）：仍显示取词高亮，但不显示悬停翻译弹窗。
- **已高亮 / 划线注释的文本**：悬停取词完全正常——注释的常驻选区不会抑制取词，无需点击页面恢复。
...
3. CSV 格式为 UTF-8 BOM（Excel 兼容），列：`word, phon, exp, add_time`。
4. phon 和 exp 内容来自 zotero-pdf-translate 的字典引擎结果。

> 通过 **导出生词本** 功能可将 CSV 导出为其他格式。

**选择生词本**：从拉取的生词本列表中选择目标词本，翻译弹窗点击 `+` 按钮时加入该词本。
...
> 建议选择「单词原型」，变形词加入生词本可能导致缺少音标和释义。此选项对欧路词典和本地生词本均生效；墨墨背单词在 App 展示层已自动归一化为词典原型。
...
| 选项 | 说明 |
| --- | --- |
| 保存翻译至注释评论 | 翻译写入注释的评论字段，注释正文保留原单词 |
